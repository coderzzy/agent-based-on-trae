...
> - `anthropic_engineering_stage1.py` / `anthropic_engineering_stage2.py`
> - `anthropic_research_stage1.py` / `anthropic_research_stage2.py`
> - `anthropic_news_stage1.py` / `anthropic_news_stage2.py`
> - `twitter_stage1.py`
> - `merge_results.py`
> - `common.py` (通用工具函数)
...
# 使用已设置的 OUTPUT_DIR 环境变量
mkdir -p "$OUTPUT_DIR/stage1"

# Engineering Stage 1
python scripts/anthropic_engineering_stage1.py \
  --html "$OUTPUT_DIR/html/anthropic_engineering.html" \
  --output "$OUTPUT_DIR/stage1/engineering.json"

# Research Stage 1
python scripts/anthropic_research_stage1.py \
  --html "$OUTPUT_DIR/html/anthropic_research.html" \
  --output "$OUTPUT_DIR/stage1/research.json"

# News Stage 1
python scripts/anthropic_news_stage1.py \
  --html "$OUTPUT_DIR/html/anthropic_news.html" \
  --output "$OUTPUT_DIR/stage1/news.json"
...
python scripts/twitter_stage1.py \
  --html "$OUTPUT_DIR/html/twitter_karpathy.html" \
  --output "$OUTPUT_DIR/stage1/twitter_karpathy.json" \
  --username karpathy \
  --display-name "Andrej Karpathy"

# Sam Altman
python scripts/twitter_stage1.py \
  --html "$OUTPUT_DIR/html/twitter_samaltman.html" \
  --output "$OUTPUT_DIR/stage1/twitter_samaltman.json" \
  --username sama \
  --display-name "Sam Altman"

# Guillermo Rauch
python scripts/twitter_stage1.py \
  --html "$OUTPUT_DIR/html/twitter_rauchg.html" \
  --output "$OUTPUT_DIR/stage1/twitter_rauchg.json" \
  --username rauchg \
  --display-name "Guillermo Rauch"

# Garry Tan
python scripts/twitter_stage1.py \
  --html "$OUTPUT_DIR/html/twitter_garrytan.html" \
  --output "$OUTPUT_DIR/stage1/twitter_garrytan.json" \
  --username garrytan \
  --display-name "Garry Tan"

# Amjad Masad
python scripts/twitter_stage1.py \
  --html "$OUTPUT_DIR/html/twitter_amasad.html" \
  --output "$OUTPUT_DIR/stage1/twitter_amasad.json" \
  --username amasad \
  --display-name "Amjad Masad"

# Peter Yang
python scripts/twitter_stage1.py \
  --html "$OUTPUT_DIR/html/twitter_peteryang.html" \
  --output "$OUTPUT_DIR/stage1/twitter_peteryang.json" \
  --username petergyang \
  --display-name "Peter Yang"

# Zara Zhang
python scripts/twitter_stage1.py \
  --html "$OUTPUT_DIR/html/twitter_zarazhang.html" \
  --output "$OUTPUT_DIR/stage1/twitter_zarazhang.json" \
...
# 保存到 $OUTPUT_DIR/html/articles/https_www_anthropic_com_engineering_xxx.html
```
...
# Engineering Stage 2
python scripts/anthropic_engineering_stage2.py \
  --stage1 "$OUTPUT_DIR/stage1/engineering.json" \
  --articles-dir "$OUTPUT_DIR/html/articles" \
  --output "$OUTPUT_DIR/stage2/engineering.json"

# Research Stage 2
python scripts/anthropic_research_stage2.py \
  --stage1 "$OUTPUT_DIR/stage1/research.json" \
  --articles-dir "$OUTPUT_DIR/html/articles" \
  --output "$OUTPUT_DIR/stage2/research.json"

# News Stage 2
python scripts/anthropic_news_stage2.py \
  --stage1 "$OUTPUT_DIR/stage1/news.json" \
  --articles-dir "$OUTPUT_DIR/html/articles" \
  --output "$OUTPUT_DIR/stage2/news.json"

# Twitter 不需要 Stage 2（内容已在 Stage 1 获取完整）
# 直接复制所有 Twitter stage1 结果到 stage2
cp "$OUTPUT_DIR/stage1/twitter_karpathy.json" "$OUTPUT_DIR/stage2/twitter_karpathy.json"
cp "$OUTPUT_DIR/stage1/twitter_samaltman.json" "$OUTPUT_DIR/stage2/twitter_samaltman.json"
cp "$OUTPUT_DIR/stage1/twitter_rauchg.json" "$OUTPUT_DIR/stage2/twitter_rauchg.json"
cp "$OUTPUT_DIR/stage1/twitter_garrytan.json" "$OUTPUT_DIR/stage2/twitter_garrytan.json"
cp "$OUTPUT_DIR/stage1/twitter_amasad.json" "$OUTPUT_DIR/stage2/twitter_amasad.json"
cp "$OUTPUT_DIR/stage1/twitter_peteryang.json" "$OUTPUT_DIR/stage2/twitter_peteryang.json"
cp "$OUTPUT_DIR/stage1/twitter_zarazhang.json" "$OUTPUT_DIR/stage2/twitter_zarazhang.json"
```
...
python scripts/merge_results.py \
  ...
  --output "$OUTPUT_DIR/ai_news_raw.json"
```

**最终输出** (`$OUTPUT_DIR/ai_news_raw.json`):
```json
{
  "date": "2026-04-06",
  "total_count": 5,
...
根据处理后的JSON数据，生成飞书文档格式的Markdown文件，保存到 `$OUTPUT_DIR/ai_news_YYYY-MM-DD.md`。

### Step 8: 生成小红书格式的 HTML 报告

根据处理后的JSON数据，生成小红书格式的HTML报告，保存到 `$OUTPUT_DIR/xiaohongshu_YYYY-MM-DD.html`。
